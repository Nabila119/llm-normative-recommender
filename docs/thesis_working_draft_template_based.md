...
The wrong-action cases in the ChatGPT baseline should be interpreted as boundary cases in the dataset design. They occurred in 6 formula rows, corresponding to 3 unique Food Ministry norms. These norms were closer to product-compliance or product-classification rules than to direct recommender-action norms. For example, a rule such as "Certified lactose free products must comply with labeling rules" concerns whether a product satisfies a labeling obligation, not whether the recommender should recommend that product to a user. Because the dataset schema and prompt emphasized recommendation actions, these rows were formalized over `recommend(System,y,x)`, which changed the deontic action. The rows were retained rather than silently removed because they reveal an important scope limitation: stakeholder policy statements do not always fit the assumed recommendation-action schema. A future version of the dataset should include a norm-scope field, such as `recommender_action`, `product_compliance`, `classification`, or `background_rule`, so that product-level obligations can either be formalized with their own action predicates, such as `O(compliesWithLabeling(y))`, or stored separately from recommender-action norms.

The lost-meaning cases show a different kind of boundary between natural-language scope and formal vocabulary. They occurred in 10 formula rows, corresponding to 5 unique natural-language norms. Most of these cases involved broad allergy-related language being narrowed to a specific nut-allergy predicate. For example, a norm referring to "allergic users" was formalized using `allergicTo(x,Nuts)`, which preserves an allergy-related topic but excludes other possible allergies. This suggests that the error was caused by the interaction between underspecified natural language and a controlled vocabulary that contained specific allergy predicates but did not consistently provide or require a general predicate such as `allergicUser(x)`. The finding motivates richer vocabulary design: canonical vocabularies should include both general predicates and specific predicates, and prompts should instruct the model not to replace a broad category with a single example unless the natural-language norm explicitly does so.
...
A second direction is to extend the human evaluation of round-trip semantic preservation. This thesis already includes human review of selected round-trip rows, including automatically flagged low-score cases and sampled high-score cases. Future work could increase the number of reviewed rows, involve more than one reviewer, and measure inter-annotator agreement. This would make the conclusions about semantic preservation stronger and would show more clearly where the automatic scoring method is reliable and where it fails.
...
Another extension is to test additional LLMs or prompt strategies for the NL-to-logic translation task. The evaluation can measure syntax validity, semantic preservation, parser error rates, correction effort, vocabulary diversity, and usefulness for downstream DJ4ME components.
...
A further future direction is to study prompt variants and correction workflows more systematically. The current prompts already produced parser-valid datasets for the reported experiments. Future work could test whether alternative prompts improve predicate consistency, stakeholder coverage, or semantic preservation. It could also evaluate a logged repair workflow in which invalid or low-scoring formulas are sent back to an LLM with parser or round-trip feedback. This would be an extension of the current pipeline, and the repair steps would need to be recorded so that improvements remain reproducible rather than hidden manual edits.
