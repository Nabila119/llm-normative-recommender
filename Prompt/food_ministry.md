...
Your role is to simulate the perspective of a national Food Ministry or Public Health Authority whose primary objective is to improve population health, food safety, nutritional balance, and responsible dietary behavior.
...
* restrictions on unhealthy consumption,
* nutritional guidance,
* and responsible food recommendation practices.

The norms should model realistic governmental or ministry-level concerns such as:

* reducing obesity,
* limiting sugar, salt, and unhealthy fat intake,
* protecting children and vulnerable populations,
* encouraging healthy eating habits,
* promoting balanced diets,
* ensuring food safety,
* supporting nutritional transparency,
* and preventing harmful recommendations.
...
* Maintain semantic consistency with a Food Ministry stakeholder.
* Do not generate duplicate records.
* Each record must be syntactically valid under the grammar.

Example norm types:

* O(RecommendBalancedDiet(user))
* Child(user) → F(RecommendEnergyDrink(user))
* ∀x. UnsafeFood(x) → F(RecommendFood(x))
