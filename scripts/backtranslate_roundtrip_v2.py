...
OUT_DIR = PROJECT_ROOT / "outputs" / "roundtrip_v2"
...
def join_english_parts(parts, conjunction):
...
def predicate_to_english(expr):
    negated = expr.startswith("¬")
    raw = expr[1:].strip() if negated else expr
...
def condition_to_english(condition):
    ...
    disjuncts = split_top_level(condition, "∨")
    if disjuncts:
        rendered = []
        for part in disjuncts:
            part = strip_outer_parentheses(part)
            text = condition_to_english(part)
            if split_top_level(part, "∧"):
                text = f"both {text}"
            rendered.append(text)
        return f"either {join_english_parts(rendered, 'or')}"

    conjuncts = split_top_level(condition, "∧")
    if conjuncts:
        rendered = [condition_to_english(part) for part in conjuncts]
        return join_english_parts(rendered, "and")

    return predicate_to_english(condition)


def conditions_to_english(conditions):
    rendered = [condition_to_english(condition) for condition in conditions]
    if len(rendered) == 1:
        return rendered[0]
    if len(rendered) == 2:
        return f"all of the following hold: {rendered[0]}; and {rendered[1]}"
    return f"all of the following hold: {'; '.join(rendered[:-1])}; and {rendered[-1]}"
...
def backtranslate_norm(normalized):
    ...
    if conditions:
        condition_text = conditions_to_english(conditions)
